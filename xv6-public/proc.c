...
int l_ticks[3] = {1, 2, 4}; // number of ticks that can excuted at one time.
int allot[3] = {5, 10, 0};  // number of ticks that stay in each level.
int mlfq_ticks = 0;         // mlfq is counted for boost up.
int total_portion = 0;      // total CPU sharing portion
struct proc * picked = 0;
int mlfq_pass = 0;          // mlfq is running under stride algorithm.
int mlfq_stride = 100;      // initial portion of mlfq is 100%
...
found:
  ...
  // added for Project2: MLFQ & Stride scheduling
  p->isShare = 0;
  ...
  // added for Project3: thread implementation
  p->tid = 0;       // the tid of the main thread is 0.
  p->main_thd = p;    // the main thread's parent is itself.
  p->thd_num = 1;  // the main thread is 1
  p->nexttid = 1;
  // to save usable stack pages later.
  for(i = 0; i < NPTHD; i++) {
    p->usable_stack[i] = 0;
  }
...
growproc(int n)
{
  ...
  // sizes of all threads should be same.
  curproc->sz = sz;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  np->isSet = curproc->main_thd->isSet;
  if(np->isSet == 0) {
    prepare_stack(np);
  }
  else {
    for(i = 0; i < NPTHD; i++) {
      np->usable_stack[i] = curproc->main_thd->usable_stack[i];
      if(curproc->tid == 0) {
        a = np->usable_stack[i];

        for(j = 0; j < 2; a += PGSIZE, j++){ //only dealloc guard, stack
          pte = walkpgdir(np->pgdir, (char*)a, 0);
          if(!pte)
...
exit(void)
{
  ...
  if(curproc->main_thd->thd_num != 1 || curproc->tid != 0
    || curproc->killed == 1) {
    // remains the worker thread or the thread calls exit()
    exit_all_thread1(curproc);
  }
  ...
  // if(curproc->tid != 0) curproc->state = UNUSED;
  sched();
  panic("zombie exit");
}
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE && p->tid == 0){
        clean_all_thread(p); // should be cleaned up all remaining thread.
        // Found one.
        pid = p->pid;
        kfree(p->kstack);
        p->kstack = 0;
...
// In Project 3, my scheduler is modified for implementation of thread.
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->isShare == 1 && p->portion == 0) {
        // cprintf("crazy process\n");
        // procdump();
        p->pass += p->stride;
        continue;
      }

      if(p->isShare == 0) { // if the mlfq proc, incresse proc tick
        p->tick++;
        mlfq_ticks++;
      ...
      // Switch to choosen process.  It is the process's job
...
int set_cpu_share(int portion) {
  ...
  myproc()->isShare = 1;
  split_portion(myproc()); // if there are threads, the portion shuold be split.
  release(&ptable.lock);
  return myproc()->pid;
}
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf("    %d %s %s tid: %d isSet: %d\n", p->pid, state, p->name, p->tid, p->isSet);
    if(p->state == SLEEPING){
